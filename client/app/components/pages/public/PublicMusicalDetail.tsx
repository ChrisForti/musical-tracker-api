...
export const PublicMusicalDetail: React.FC = () => {
  ...
  const fetchMusicalData = async () => {
    try {
      const [musicalRes, performancesRes, rolesRes, castingsRes] = await Promise.all([
  ...
  const getCastForPerformance = (performanceId: number) => {
    return castings.filter(casting => casting.performanceId === performanceId);
  };

  const upcomingPerformances = performances.filter(perf => new Date(perf.date) >= new Date());
  const pastPerformances = performances.filter(perf => new Date(perf.date) < new Date());
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-center h-64">
            <div className="text-lg text-gray-600 dark:text-gray-400">Loading musical details...</div>
          </div>
        </div>
      </div>
    );
            ...
            <div className="p-6 md:w-2/3">
              ...
              <div className="space-y-2 mb-4">
                <div className="text-gray-700 dark:text-gray-300">
                  <span className="font-medium">Music by:</span> {musical.composer}
                </div>
                <div className="text-gray-700 dark:text-gray-300">
                  <span className="font-medium">Lyrics by:</span> {musical.lyricist}
                </div>
              </div>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {upcomingPerformances.map((performance) => {
                  ...
                  return (
                    <div key={performance.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                      <div className="flex flex-col md:flex-row md:items-center justify-between mb-2">
                        <div>
                          ...
                          <div className="text-sm text-gray-500 dark:text-gray-500">
                            {performance.theater.address}, {performance.theater.city}
                          </div>
                        </div>
                      </div>
                      ...
                      {cast.length > 0 && (
                        <div className="mt-3">
                          <div className="text-sm font-medium text-gray-900 dark:text-white mb-2">Cast:</div>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                            {cast.map((casting) => (
                              <div key={casting.id} className="text-gray-600 dark:text-gray-400">
                                <span className="font-medium">{casting.role.characterName}:</span> {casting.actor.name}
                              </div>
                            ))}
                          </div>
                        </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {pastPerformances.slice(0, 5).map((performance) => (
                  <div key={performance.id} className="flex justify-between items-center py-2 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                    <div>
                      <span className="text-gray-900 dark:text-white">
                        {new Date(performance.date).toLocaleDateString()}
                      </span>
                      <span className="text-gray-600 dark:text-gray-400 ml-2">
                        at {performance.theater.name}, {performance.theater.city}
                      </span>
                    </div>
                  </div>
                ))}
