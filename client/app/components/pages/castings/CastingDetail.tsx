...
export function CastingDetail() {
  ...
  const fetchCastingDetails = async () => {
    ...
    try {
      ...
      // Fetch the casting
      const castingResponse = await fetch(
      ...
      const [actorRes, roleRes, performanceRes] = await Promise.all([
        ...
        fetch(
          `http://localhost:3000/v2/performance/${castingData.performanceId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
      ...
      if (roleRes.ok) {
        ...
        if (roleData.musicalId) {
          const musicalRes = await fetch(
            `http://localhost:3000/v2/musical/${roleData.musicalId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
  ...
  const handleDelete = async () => {
    if (
      !casting ||
      !confirm("Are you sure you want to delete this casting assignment?")
    ) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/v2/casting/${casting.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
