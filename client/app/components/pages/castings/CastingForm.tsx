...
export function CastingForm() {
  ...
  const fetchCasting = async (castingId: string) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`http://localhost:3000/v2/casting/${castingId}`, {
        headers: {
      ...
      const performance = performances.find(p => p.id === casting.performanceId);
      if (performance) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedRole = roles.find(r => r.id === formData.roleId);
    const selectedPerformance = performances.find(p => p.id === formData.performanceId);
    
    if (selectedRole && selectedPerformance && selectedRole.musicalId !== selectedPerformance.musicalId) {
      setError('The selected role must belong to the same musical as the performance');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to ${isEditing ? 'update' : 'create'} casting`);
      }
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">
              {isEditing ? 'Edit Cast Assignment' : 'Create New Cast Assignment'}
            </h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="px-6 py-4 space-y-6">
            ...
            <div>
              <label htmlFor="musicalFilter" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            ...
            {/* Actor Selection */}
            <div>
              <label htmlFor="actorId" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Actor *
              </label>
              <select
                id="actorId"
            ...
            <div>
              <label htmlFor="performanceId" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Performance *
              </label>
              <select
                id="performanceId"
        ...
        <div className="mt-6 text-sm text-gray-600 dark:text-gray-400">
          ...
          <ul className="list-disc list-inside space-y-1">
            <li>Select a musical first to filter relevant performances and roles</li>
